...
internal sealed class NdiReceiver : IVideoSignalReceiver
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            NdiLog.ReceiverFailed(logger, exception.GetType().Name);
        }
    ...
    private async Task StartCoreAsync(VideoSignalSourceDescriptor source, CancellationToken cancellationToken)
    {
        ...
        if (!interop.TryOpenReceiver(source.Id, out INdiReceiverSession receiver, out string? errorType))
        {
            SetState(VideoSignalState.Faulted, errorType);
            NdiLog.ReceiverFailed(logger, errorType ?? "OpenFailed");
            return;
        }
    ...
    private async Task ReceiveLoopAsync(CancellationToken startCancellation)
    {
        ...
        try
        {
            ...
            {
                INdiReceiverSession? active = Volatile.Read(ref session);
                if (active is null)
                {
                    return;
                }

                if (active.TryReceive(out NdiReceivedFrame frame))
                {
